...
for j in ratio:
    ...
    rmse = np.zeros(M)
    rel_rmse = np.zeros(M)
    train_set, test_set = train_test_split(train, test_size=1-j, random_state=1) # Attempt with random selection of points
...
# Separate tuples for training for LSTM
    ...
    loader = data.DataLoader(data.TensorDataset(X_train, y_train), shuffle=True, batch_size=50) # batch size is changeable
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            y_pred = model(X_test)
            rmse[epoch] = np.sqrt(loss_fn(y_pred, y_test))
            rel_rmse[epoch] = np.sqrt(len(y_test)*loss_fn(y_pred, y_test)/sum((i[-1])**2 for i in y_test)) 
